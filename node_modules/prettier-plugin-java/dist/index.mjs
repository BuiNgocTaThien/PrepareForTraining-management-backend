...
function hasDanglingComments(node) {
	return node.comments?.some(({ leading, trailing }) => !leading && !trailing) ?? false;
}
...
function printAssignment(leftDoc, operator, rightDoc, rightNode) {
	...
	if (rightNode.type === "binary_expression" || rightNode.type === "instanceof_expression" || rightNode.type === "ternary_expression" && (rightNode.conditionNode.type === "binary_expression" || rightNode.conditionNode.type === "instanceof_expression") || isMember(rightNode) && hasDanglingComments(rightNode) || !isMember(rightNode) && hasLeadingComments(rightNode)) return group$7([
		leftDoc,
		operator,
...
function parentNeedsParentheses(path) {
	...
	switch (parent?.type) {
		...
		case "throw_statement": if (willReturnOrThrowStatementBreak(path)) return false;
	}
}
...
function findEmbeddedLanguage(path) {
	return [path.node, ...path.ancestors].find(({ type, comments }) => type === "block" || comments?.some(({ leading }) => leading))?.comments?.filter(({ leading }) => leading).map(({ value }) => value.match(/^(?:\/\/|\/\*)\s*language\s*=\s*(\S+)/)?.[1]).findLast((language) => language)?.toLowerCase();
}
...
function canPrintParamsWithoutParens(node) {
	return node.parametersNode.type === "identifier" && !hasDanglingComments(node) && !node.parametersNode.comments;
}
		...
		path.each((child) => {
			switch (child.node.type) {
				...
				case "marker_annotation":
					...
					break;
				case "identifier":
				case "scoped_identifier": identifier.push(print(child));
			}
		}, "namedChildren");
...
function handleLambdaExpressionComments(commentNode) {
	...
	if (enclosingNode?.type === "lambda_expression" && precedingNode && followingNode && enclosingNode.children.find(({ type }) => type === "->").end.index < commentNode.start.index) {
		if (followingNode.type === "block") {
			if (followingNode.namedChildren.length) util.addLeadingComment(followingNode.namedChildren[0], commentNode);
...
function printLeadingComment(path) {
	...
	if (comment.type === "block_comment") {
		let lineBreak = " ";
		if (hasNewline(originalText, parser_default.locEnd(comment))) {
			if (hasNewline(originalText, parser_default.locStart(comment), { backwards: true })) lineBreak = hardline;
			else lineBreak = line;
		}
	...
	node.children.forEach((child, index) => {
		...
		(child.type === "parenthesized_expression" && !isParenthesizedParent(javaNode) ? child.namedChildren : [child]).forEach((child) => {
			...
			if (type === "block_comment" || type === "line_comment") comments.push({
				...
				if (fieldName) {
					if (multiFields?.[fieldName]) javaNode[`${fieldName}Nodes`].push(javaChild);
