...
const { group: group$7, hardline: hardline$6, ifBreak: ifBreak$2, indent: indent$7, indentIfBreak: indentIfBreak$2, join: join$7, line: line$7, lineSuffixBoundary: lineSuffixBoundary$1, softline: softline$4 } = prettier_doc.builders;
...
const { group: group$6, hardline: hardline$5, ifBreak: ifBreak$1, indent: indent$6, indentIfBreak: indentIfBreak$1, join: join$6, line: line$6, softline: softline$3 } = prettier_doc.builders;
...
const { group: group$5, hardline: hardline$4, indent: indent$5, join: join$5, line: line$5, softline: softline$2 } = prettier_doc.builders;
var classes_default = {
...
const { align, breakParent: breakParent$1, conditionalGroup, dedent, group: group$4, hardline: hardline$3, ifBreak, indent: indent$4, indentIfBreak, join: join$4, line: line$4, lineSuffixBoundary, softline: softline$1 } = prettier_doc.builders;
...
const { group: group$2, hardline: hardline$2, indent: indent$2, join: join$2, softline } = prettier_doc.builders;
...
const { group, indent, join, line: line$1 } = prettier_doc.builders;
...
function handleLambdaExpressionComments(commentNode) {
	...
	if (enclosingNode?.type === "lambda_expression" && precedingNode && followingNode && enclosingNode.children.find(({ type }) => type === "->").end.index < commentNode.start.index) {
		if (followingNode.type === "block") {
			if (followingNode.namedChildren.length) prettier.util.addLeadingComment(followingNode.namedChildren[0], commentNode);
			else prettier.util.addDanglingComment(followingNode, commentNode, void 0);
		} else prettier.util.addLeadingComment(followingNode, commentNode);
		return true;
	}
}
...
const parser = (async () => {
	...
	const Java = await web_tree_sitter.Language.load(new URL("./tree-sitter-java_orchard.wasm", require("url").pathToFileURL(__filename).href));
